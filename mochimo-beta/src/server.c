   ...
   while(Running) {
      ...
      for(np = Nodes; np < Hi_node; np++) {
         ...
         if(np->opcode == OP_FOUND) {
            ...
            if(status == 0) addrecent(np->src_ip);
         }
      ...
      if(Contend_ip && (Ltime - Contend_time) >= LULL)
         contend(Contend_ip);  /* there was contention -- RESTART */
      ...
      if(Bcpid == 0 && Blockfound == 0
         ...
         lfd = lock("txq1.lck", 10);
         if(lfd == -1) goto skipbc;
