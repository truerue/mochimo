...
int read_coreipl(char *fname)
{
   ...
   for(j = 0; j < CORELISTLEN; ) {
      ...
      if(*buff == '#') continue;
      addrstr = strtok(buff, " \r\n\t");
      if(Trace) plog("   parse: %s", addrstr);  /* debug */
...
word32 init_coreipl(NODE *np, char *fname)
{
   ...
   for(j = 0 ; j < RPLISTLEN; j++) {
      ...
      if(ip == 0) break;  /* end of list */
      if(Trace)
         plog("read_coreipl() about to call get_ipl(%s)", ntoa((byte *) &ip));
...
void add_weight(byte *weight, int difficulty)
{
   ...
   temp[0] = difficulty;
   multi_add32(weight, temp, weight, 8);
   if(Trace)
 ...
 *
 * Error codes 1-8 are validation errors; codes >= 100 are I/O, 
...
byte *tfval(char *fname, byte *highblock, int weight_only, int *result)
{
   ...
   *result = 100;                 /* I/O high error code */
   ...
   for(gblock = 1; ; gblock = 0) {
      ...
      if(highblock[0]) {
         if(stemp <= time1)  /* unsigned time here */
            break;
      }
...
skipval:
      ...
      if(highblock[0] != 0xff) {
         ...
         if(Trace) plog("new difficulty: %d", difficulty);  /* debug */
      }
next:
...
int trim_tfile(byte *highbnum)
{
   ...
   error("tfile(): flag = %d  bnum = 0x%s", flag, bnum2hex(bnum));  /* debug */
...
top:
   ...
   for(j = 1; j < Quorum && Running; ) {
      ...
      result2 = memcmp(np->tx.weight, highweight, HASHLEN);
   ...
   for(k = 0; k < Quorum && Running; k++) {
      ...
      if(memcmp(tfweight, highweight, HASHLEN) < 0) goto try_again;
      break;  /* success */
   }
   ...
   if(cmp64(bnum, Cblocknum) != 0) {
      error("init(): line %d", __LINE__);  /* should not happen */
      goto try_again;
   }
...
try_again:
   ...
   if(Trace) {
      if(tfweight)
         plog("tfweight = 0x%s...", addr2str(tfweight));    /* debug */
...
int init(void)
{
   ...
   /* read into Coreplist[] and shuffle  */
