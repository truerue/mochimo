...
int send_found(void)
{
   ...
   if(Cblocknum[0] == 0) {
      ecode = 1;
      /* Back up out Cblocknum in child only to 0x...ff block. */
...
bad:
   ...
   /* Send found message to all recent peer's */
...
int update(char *fname, int mode)
{
   ...
   /* no-one should have a lock on txq1.lck  -- make sure */
   if((lfd = lock("txq1.lck", 1)) == -1)
      return error("update(): txq1.lck still locked");
   unlock(lfd);
   ...
   if(!Ininit) {
      ...
      if(tag_build()) goto err;  /* rebuild tag index */
   }
   plog("Block %s: 0x%s", mode ? "solved" : "updated", bnum2hex(Cblocknum));
   if(!Ininit)
      printf("Solved: %u  Haiku/second: %u  Difficulty: %d\n",
